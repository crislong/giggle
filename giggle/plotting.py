...
m1c = mf.momentoneC(gx, gy, ms, md , p, m, 1, beta, 1, 100, ap, incl, 0)
m1k = mf.momentoneC(gx, gy, ms, 0 , p, m, 1, beta, 1, 100, ap, incl, 0)
for i in range(len(car)):
    for j in range(len(car)):
        grid_angle[i,j] = math.atan2(car[i], car[j])
spir = mf.perturbed_sigma(g_r, grid_angle, p, 1, 100, md, beta, m, ap,0)
